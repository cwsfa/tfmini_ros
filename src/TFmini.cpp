...
namespace benewake{
    TFmini::TFmini(std::string _name, int _baudRate) : // init TFmini device
    ...
    bool TFmini::readData(unsigned char* _buf, int _nRead){ // Read data through serial communication
      int total = 0, ret = 0;
    ...
    float TFmini::getDist(){ // Get distance from TFmini
      while(true){
    ...
    void TFmini::closePort(){ // Close serial port
      close(serial_);
    }
}